...
function listVersions() {
    ...
    return _.chain(folders)
        .map(function(tag) {
            ...
            // Is it dijetspub?
            if (pkg.name != 'dijetspub') return null;
...
// Remove an installed version of dijetspub
...
function loadVersion(version) {
    ...
    .then(function(resolved) {
        ...
        try {
            dijetspub = require(resolved.path);
        } catch (err) {
            console.log(color.red('Error loading version '+resolved.tag+': '+(err.stack || err.message || err)));
            return null;
        }

        if (!dijetspub) throw new Error('DijetsPub Version '+resolved.tag+' is corrupted');
        return dijetspub;
    });
}
...
function linkVersion(name, folder) {
    if (!name) return Q.reject(new Error('Require a name to represent this DijetsPub version'));
