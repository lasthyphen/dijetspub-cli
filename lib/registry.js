...
function availableVersions() {
    return initNPM()
    .then(function() {
        return Q.nfcall(npm.commands.view, ['dijetspub', 'versions', 'dist-tags'], true);
    })
...
function resolveVersion(version) {
    ...
    .then(function(available) {
        ...
        // Check version
        if (!version) throw new Error('Invalid version or tag "'+_version+'", see available using "dijetspub ls-remote"');
        return version;
    });
}

// Install a specific version of dijetspub
function installVersion(version, forceInstall) {
    ...
    .spread(function(tmpDir) {
        ...
        console.log('Installing DijetsPub', version);
        return Q.nfcall(npmi.bind(npmi), options).thenResolve(tmpDir);
    })
    .then(function(tmpDir) {
        var dijetspubRoot = path.resolve(tmpDir, 'node_modules/dijetspub');
        var packageJson = fs.readJsonSync(path.resolve(dijetspubRoot, 'package.json'));
        var version = packageJson.version;

        var outputFolder = path.resolve(config.VERSIONS_ROOT, version);

        if (!tags.isValid(version)) throw 'Invalid DijetsPub version, should satisfies '+config.DIJETSPUB_VERSION;

        // Copy to the install folder
        return Q.nfcall(fs.copy.bind(fs), dijetspubRoot, outputFolder)
        .thenResolve(version);
    });
}
